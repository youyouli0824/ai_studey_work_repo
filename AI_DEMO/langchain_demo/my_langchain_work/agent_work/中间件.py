...
my_agent=create_agent(
    ...
    middleware=[
        ...
        HumanInTheLoopMiddleware({
            ...
            "query_order_status":{"allowed_decisions":["approve","reject"]},
            "company_refund_policy":{"allowed_decisions":["approve","reject"]},
            "get_age":{"allowed_decisions":["approve","reject"]},
        })
    ],
...
for query in queries:
    ...
    result = my_agent.invoke(inputs, config=config)

    # 安全判断：存在中断才需要人工审核
    if "__interrupt__" in result:
        tool_info = result['__interrupt__'][0].value['action_requests'][0]
        tool_name = tool_info['name']
        app_or_reject = input(f"请确认调用{tool_name}（输入 approve 同意 / reject 拒绝）：")
        # 中英文兼容转换
        if app_or_reject.strip() in ["同意", "是", "yes", "1"]:
            app_or_reject = "approve"
        elif app_or_reject.strip() in ["拒绝", "否", "no", "0"]:
            app_or_reject = "reject"
        # 恢复Agent执行
        res = my_agent.invoke(
